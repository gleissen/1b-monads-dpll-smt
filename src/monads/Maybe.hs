...
-- The Maybe monad:
-----------------------------------------------------------
-- As discussed in class, the Maybe monad allows to compose computations that
-- may fail. 
--
-- That is, the bind (>>=) operator applies functions until one of the
-- applications fails, at which point the computation chain evaluates to
-- Nothing, from there on.
--
-- If at a loss for what to do, check out the work-group lecture slides!
-----------------------------------------------------------
